...
ggplot() + 
  #geom_sf(data = grid_iNatUY_GIS %>% mutate(N=ifelse(spatialIntensity==0, NA, rescale(spatialIntensity, to = c(0, 1)))), aes(fill=spatialIntensity)) +
  #geom_sf(data = grid_iNatUY_GIS %>% mutate(N=ifelse(temporalIntensity==0, NA, rescale(temporalIntensity, to = c(0, 1)))), aes(fill=temporalIntensity)) +
...
grid_iNatUY_GIS %>% mutate(index=temporalIntensity+spatialIntensity) %>% 
  ...
  scale_fill_fermenter(palette = 'Spectral', direction = 1, na.value="#ede8e8", n.breaks=8) +
  geom_sf(data = Uruguay, color='black', fill=NA) +
  theme_bw() +
