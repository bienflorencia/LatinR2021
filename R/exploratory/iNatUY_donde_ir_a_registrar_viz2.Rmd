...
# grupos <- c("Todos", "Aves", "Mammalia", "Amphibia", "Animalia", "Plantae", 
#             "Mollusca", "Insecta", "Arachnida", "Fungi", "Reptilia", 
#             "Actinopterygii", "Chromista", "Protozoa")

grupos <- names(datos)[c(-1:-9, -ncol(datos))] %>% gsub('.*_', '', .) %>% unique %>% sort

gr <- c('', paste0('_', grupos[-1]))
...
for (i in 1:length(grupos)) {
  base <- base %>% 
    ...
    weight = .5, 
    color = 'white',
    fillColor = pal(datos[paste0('ranking', gr[i])][[1]]),
    fillOpacity = .5,
    popup = mkpopup(datos, grupos[i]),
    highlightOptions = highlightOptions(color = "white", 
                                        weight = 5, 
                                        fillOpacity = .8,
                                        bringToFront = TRUE),
...
base %>% 
  # Control de capas:
  addLayersControl(
    baseGroups = c("Open Street Map", "Open Topo Map", "Imagen", grupos),
